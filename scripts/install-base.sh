...
FQDN='playground.dre-codes'
USER='senor-dre'
USER_PASSWORD=$(/usr/bin/openssl passwd -crypt 'admin')
ROOT_PASSWORD=$(/usr/bin/openssl passwd -crypt 'vagrant')
KEYMAP='us'
...
#   /usr/bin/useradd  --password ${USER_PASSWORD} --comment '$USER User' --create-home --user-group $USER
#   /usr/bin/passwd -d $USER
#   echo ">>>> ${CONFIG_SCRIPT_SHORT}: Configuring sudo.."
#   echo 'Defaults env_keep += "SSH_AUTH_SOCK"' > /etc/sudoers.d/20_$USER
#   echo '$USER ALL=(ALL) NOPASSWD: ALL' >> /etc/sudoers.d/20_$USER
#   /usr/bin/chmod 0440 /etc/sudoers.d/20_$USER
#   echo ">>>> ${CONFIG_SCRIPT_SHORT}: Configuring ssh access for $USER.."
#   /usr/bin/install --directory --owner=vagrant --group=vagrant --mode=0700 /home/$USER/.ssh
